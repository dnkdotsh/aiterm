...
def mock_dependencies(mocker):
    """
    A fixture to automatically mock handler functions and their direct dependencies.
    This isolates tests to the logic within the handler functions themselves.
    """
    ...
    # Mock the workflow module, and configure a default return for image generation
    mock_workflows = mocker.patch("src.aiterm.handlers.workflows")
    mock_workflows._perform_image_generation.return_value = (True, "/fake/path.png")
...
class TestHandlers:
    ...
    def test_handle_load_session(self, mock_dependencies, mocker):
        ...
        filepath_str = "/path/to/session.json"
    ...
    def test_handle_multichat_session(self, mock_dependencies):
        ...
        call_args, _ = handlers.MultiChatUI.call_args
        assert call_args[2] == "compare these"
    ...
    def test_handle_image_generation(self, mock_dependencies):
        ...
        call_args, call_kwargs = handlers.workflows._perform_image_generation.call_args
        assert call_args[1] == "dall-e-test"  # model
        assert call_args[2] == "a cat"  # prompt
        assert call_kwargs["session_raw_logs"] is not None  # srl_list
    ...
    def test_handle_multichat_session_with_attachments(self, mock_dependencies):
